...
class ConversationController extends Controller
{
    ...
    public function store(Request $request, ChatService $chat)
    {
        ...
        $data = $request->validate([
            ...
            'user_id' => ['required_if:type,direct', 'integer', Rule::exists('users', 'id')->where('status', 'active')->whereNull('deleted_at')],
            'name' => ['required_if:type,group', 'string', 'max:150'],
            'description' => ['nullable', 'string', 'max:2000'],
            'privacy' => ['nullable', 'in:public,private'],
            'member_ids' => ['nullable', 'array'],
            'member_ids.*' => ['integer', Rule::exists('users', 'id')->where('status', 'active')->whereNull('deleted_at')],
        ]);
    ...
    public function show(Request $request, Conversation $conversation, ChatService $chat)
    {
        ...
        return response()->json([
            'data' => $conversation->load(['activeMembers.user:id,name,email,avatar', 'owner:id,name']),
        ]);
    }
