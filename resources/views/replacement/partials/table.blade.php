...
<x-ui.table>
    <x-ui.table-header class="bg-muted/20">
        <x-ui.table-row class="border-b border-border/60">
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Replacement</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Original order</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Item</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Requested</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Ship by</x-ui.table-head>
    ...
    <x-ui.table-body>
        @forelse($rows as $record)
            ...
            <x-ui.table-row class="border-b border-border/40 group hover:bg-primary/[0.02] transition-colors">
                <x-ui.table-cell>
                    <span class="text-sm font-mono font-black text-primary">{{ data_get($r, 'number') ?? data_get($r, 'replacement_number', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-bold">{{ data_get($r, 'order_number') ?? data_get($r, 'original_order', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-medium line-clamp-2 max-w-[200px]">{{ data_get($r, 'item_name') ?? data_get($r, 'sku', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php $c = data_get($r, 'created_at'); @endphp
                    <span class="text-xs font-bold">{{ $c ? \Illuminate\Support\Carbon::parse($c)->format('M j, Y') : '—' }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php $sb = data_get($r, 'ship_by'); @endphp
                    <span class="text-xs font-bold">{{ $sb ? \Illuminate\Support\Carbon::parse($sb)->format('M j, Y') : '—' }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php
            ...
            <x-ui.table-row>
                <x-ui.table-cell colspan="7" class="h-60 text-center">
                    <div class="flex flex-col items-center justify-center gap-3 opacity-40">
                        ...
                        <p class="text-sm font-black uppercase tracking-[0.2em]">No replacements found</p>
