...
<x-ui.table>
    <x-ui.table-header class="bg-muted/20">
        <x-ui.table-row class="border-b border-border/60">
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Campaign</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Audience</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 text-center">Sent</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 text-center">Open rate</x-ui.table-head>
    ...
    <x-ui.table-body>
        @forelse($rows as $record)
            ...
            <x-ui.table-row class="border-b border-border/40 group hover:bg-primary/[0.02] transition-colors">
                <x-ui.table-cell>
                    <span class="text-sm font-black">{{ data_get($r, 'name') ?? data_get($r, 'subject', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-medium text-muted-foreground">{{ data_get($r, 'audience') ?? data_get($r, 'list_name', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell class="text-center">
                    <span class="text-sm font-black tabular-nums">{{ data_get($r, 'sent_count') ?? data_get($r, 'recipients', '0') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell class="text-center">
                    <span class="text-sm font-black text-emerald-500">{{ data_get($r, 'open_rate') ?? data_get($r, 'opens_percent', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php
            ...
            <x-ui.table-row>
                <x-ui.table-cell colspan="6" class="h-60 text-center">
                    <div class="flex flex-col items-center justify-center gap-3 opacity-40">
                        ...
                        <p class="text-sm font-black uppercase tracking-[0.2em]">No email campaigns found</p>
