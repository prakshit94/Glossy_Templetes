<div x-data="{ open: false }" class="relative" @keydown.escape.prevent="open = false" @click.outside="open = false">
    <button @click="open = !open" class="group relative inline-flex items-center justify-center rounded-xl size-10 text-muted-foreground hover:bg-secondary/40 hover:text-primary transition-all duration-300 active:scale-90">
    ...
    <div x-show="open" 
        ...
        <div class="p-6 pb-2 transition-all">
            ...
            <div class="grid grid-cols-3 gap-2 bg-secondary/20 p-1 rounded-2xl border border-border/60 backdrop-blur-md">
                <button @click="theme = 'light'" 
                    ...
                    :class="theme === 'light' ? 'bg-background text-primary shadow-lg ring-1 ring-border/60' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/40'">
                ...
                <button @click="theme = 'dark'" 
                    class="group flex flex-col items-center justify-center rounded-xl py-3 gap-2 transition-all duration-300"
                    :class="theme === 'dark' ? 'bg-background text-primary shadow-lg ring-1 ring-border/60' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/40'">
                ...
                <button @click="theme = 'system'" 
                    class="group flex flex-col items-center justify-center rounded-xl py-3 gap-2 transition-all duration-300"
                    :class="theme === 'system' ? 'bg-background text-primary shadow-lg ring-1 ring-border/60' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/40'">
                ...
                <template x-for="c in ['zinc', 'blue', 'violet', 'rose', 'orange', 'green']" :key="c">
                     <button @click="colorTheme = c" class="group relative flex items-center justify-center p-1 rounded-2xl transition-all hover:bg-secondary" :title="c">
                        <div class="size-10 rounded-xl shadow-lg transition-all border-2 border-transparent group-hover:scale-110" 
                             :class="{
                                'bg-zinc-900 dark:bg-zinc-100': c === 'zinc',
                                'bg-blue-500': c === 'blue',
                                'bg-violet-500': c === 'violet',
                                'bg-rose-500': c === 'rose',
                                'bg-orange-500': c === 'orange',
                                'bg-green-500': c === 'green',
                                'ring-2 ring-primary ring-offset-4 ring-offset-background': colorTheme === c
                             }">
                        </div>
