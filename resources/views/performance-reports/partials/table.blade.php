...
<x-ui.table>
    <x-ui.table-header class="bg-muted/20">
        <x-ui.table-row class="border-b border-border/60">
            ...
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">KPI scope</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Period</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Score</x-ui.table-head>
    ...
    <x-ui.table-body>
        @forelse($rows as $record)
            ...
            <x-ui.table-row class="border-b border-border/40 group hover:bg-primary/[0.02] transition-colors">
                ...
                <x-ui.table-cell>
                    <span class="text-xs font-medium">{{ data_get($r, 'scope') ?? data_get($r, 'team', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-bold">{{ data_get($r, 'period', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-lg font-black tabular-nums">{{ data_get($r, 'score') ?? data_get($r, 'rating', '—') }}</span>
                </x-ui.table-cell>
            ...
            <x-ui.table-row>
                <x-ui.table-cell colspan="5" class="h-60 text-center">
                    <div class="flex flex-col items-center justify-center gap-3 opacity-40">
                        ...
                        <p class="text-sm font-black uppercase tracking-[0.2em]">No performance reports yet</p>
