...
<x-ui.table>
    <x-ui.table-header class="bg-muted/20">
        <x-ui.table-row class="border-b border-border/60">
            ...
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 text-center">Rating</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Summary</x-ui.table-head>
            ...
            <x-ui.table-row class="border-b border-border/40 group hover:bg-primary/[0.02] transition-colors">
                <x-ui.table-cell>
                    <div class="flex flex-col min-w-0 max-w-[200px]">
                        <span class="text-sm font-black text-foreground truncate">{{ data_get($r, 'subject') ?? data_get($r, 'product.name', '—') }}</span>
                        <span class="text-[9px] font-mono text-muted-foreground/40 truncate">#{{ data_get($r, 'id', '—') }}</span>
                    </div>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-bold text-foreground truncate">{{ data_get($r, 'customer_name') ?? data_get($r, 'customer.name', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell class="text-center">
                    <span class="text-sm font-black text-amber-500">{{ data_get($r, 'rating', '—') }}</span>
                    <span class="text-[9px] text-muted-foreground block">/ 5</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-medium text-muted-foreground line-clamp-2 max-w-xs">{{ data_get($r, 'body') ?? data_get($r, 'comment', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php $d = data_get($r, 'created_at') ?? data_get($r, 'reviewed_at'); @endphp
                    <span class="text-xs font-bold">{{ $d ? \Illuminate\Support\Carbon::parse($d)->format('M j, Y') : '—' }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php
            ...
            <x-ui.table-row>
                <x-ui.table-cell colspan="7" class="h-60 text-center">
                    <div class="flex flex-col items-center justify-center gap-3 opacity-40">
                        ...
                        <p class="text-sm font-black uppercase tracking-[0.2em]">No reviews found</p>
