<x-layouts.app>
    ...
    <div class="p-6 lg:p-10" x-data="{
        ...
        toggleAll() {
            if (this.allSelected) {
                this.selectedCustomers = Array.from(
        ...
        toggleCustomer(id) {
            if (this.selectedCustomers.includes(id)) {
                this.selectedCustomers = this.selectedCustomers.filter(c => c !== id);
            } else {
        ...
        async performSearch() {
            ...
            const res = await fetch(
                `{{ route('customers.index') }}?search=${this.search}&filter=${this.filter}&perPage=${this.perPage}&status=${this.statusFilter}`,
                {
                    headers: {
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div class="group relative p-6 rounded-3xl bg-card/40 border border-border/60 backdrop-blur-xl hover:bg-primary/5 transition-all duration-500 overflow-hidden shadow-2xl">
                ...
                <div class="flex items-center gap-5 relative z-10">
                    ...
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 mb-1">Total Customers</p>
            ...
            <div class="group relative p-6 rounded-3xl bg-card/40 border border-border/60 backdrop-blur-xl hover:bg-emerald-500/5 transition-all duration-500 overflow-hidden shadow-2xl">
                ...
                <div class="flex items-center gap-5 relative z-10">
                    ...
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 mb-1">Active Customers</p>
            ...
            <div class="group relative p-6 rounded-3xl bg-card/40 border border-border/60 backdrop-blur-xl hover:bg-orange-500/5 transition-all duration-500 overflow-hidden shadow-2xl">
                ...
                <div class="flex items-center gap-5 relative z-10">
                    <div class="size-14 rounded-2xl bg-gradient-to-tr from-orange-500/20 to-orange-500/5 border border-orange-500/10 text-orange-500 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-500">
                        <x-ui.icon name="archive" size="7" />
                    </div>
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 mb-1">Archived</p>
                        <div class="text-3xl font-black tracking-tighter text-foreground">{{ number_format($stats['trashed'] ?? 0) }}</div>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <div class="flex bg-muted/50 px-4 py-1.5 rounded-xl border border-border/50 shadow-inner">
                                <span class="text-xs font-bold text-primary tracking-widest uppercase">Customer Registry</span>
                            </div>
                            ...
                            <div x-show="selectedCustomers.length > 0" x-cloak x-transition
                                 ...
                                 <x-ui.dropdown>
                                    <x-slot name="trigger">
                                        <x-ui.button variant="outline" size="sm" class="rounded-xl border-primary/20 bg-primary/5 text-primary font-bold shadow-sm whitespace-nowrap">
                                            <span x-text="selectedCustomers.length"></span> Selected
                                    ...
                                    <x-slot name="content">
                                        ...
                                        <div class="p-1 space-y-1">
                                            <template x-if="filter !== 'trashed'">
                                                <div class="space-y-1">
                                                    <form action="{{ route('customers.bulk-status') }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="ids" :value="JSON.stringify(selectedCustomers)">
                                                    ...
                                                    <form action="{{ route('customers.bulk-delete') }}" method="POST" onsubmit="return confirm('Archive selected customers?')">
                                                        @csrf
                                                        <input type="hidden" name="ids" :value="JSON.stringify(selectedCustomers)">
                                            ...
                                            <template x-if="filter === 'trashed'">
                                                <div class="space-y-1">
                                                    <form action="{{ route('customers.bulk-restore') }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="ids" :value="JSON.stringify(selectedCustomers)">
                                                        <button type="submit" class="w-full text-left px-3 py-2 text-[10px] font-black hover:bg-emerald-500/10 rounded-xl flex items-center text-emerald-600 uppercase tracking-widest transition-colors">
                                                    ...
                                                    <form action="{{ route('customers.bulk-force-delete') }}" method="POST" onsubmit="return confirm('PERMANENTLY delete selected records?')">
                                                        @csrf
                                                        <input type="hidden" name="ids" :value="JSON.stringify(selectedCustomers)">
                                                        <button type="submit" class="w-full text-left px-3 py-2 text-[10px] font-black hover:bg-destructive/10 rounded-xl flex items-center text-destructive uppercase tracking-widest transition-colors">
                        ...
                        <div class="flex flex-wrap items-center gap-2 w-full lg:w-auto">
                            ...
                            <a href="{{ route('customers.create') }}" class="w-full sm:w-auto mt-2 sm:mt-0">
            ...
            <x-ui.card-content class="p-0 relative min-h-[400px]">
                <div x-show="isLoading" x-cloak class="absolute inset-0 z-50 bg-background/40 backdrop-blur-sm flex items-center justify-center animate-in fade-in duration-300">
                    <div class="flex flex-col items-center gap-4">
                        ...
                        <span class="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Syncing Customer Data</span>
                    </div>
                </div>
