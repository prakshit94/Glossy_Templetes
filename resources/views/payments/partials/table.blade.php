...
<x-ui.table>
    <x-ui.table-header class="bg-muted/20">
        <x-ui.table-row class="border-b border-border/60">
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Payment</x-ui.table-head>
            ...
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Method</x-ui.table-head>
    ...
    <x-ui.table-body>
        @forelse($rows as $record)
            ...
            <x-ui.table-row class="border-b border-border/40 group hover:bg-primary/[0.02] transition-colors">
                <x-ui.table-cell>
                    <span class="text-sm font-mono font-black">{{ data_get($r, 'number') ?? data_get($r, 'payment_number', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-medium text-muted-foreground truncate max-w-[140px] block">{{ data_get($r, 'reference') ?? data_get($r, 'transaction_id', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-bold">{{ data_get($r, 'customer_name') ?? data_get($r, 'customer.name', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell class="text-right">
                    <span class="text-sm font-black tabular-nums">${{ number_format((float) (data_get($r, 'amount') ?? data_get($r, 'total', 0)), 2) }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-[10px] font-black uppercase tracking-widest">{{ data_get($r, 'method') ?? data_get($r, 'payment_method', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php $pd = data_get($r, 'paid_at') ?? data_get($r, 'created_at'); @endphp
        ...
        @empty
            <x-ui.table-row>
                <x-ui.table-cell colspan="8" class="h-60 text-center">
                    <div class="flex flex-col items-center justify-center gap-3 opacity-40">
                        <x-ui.icon :name="$moduleIcon ?? 'package'" size="12" />
                        <p class="text-sm font-black uppercase tracking-[0.2em]">No payments found</p>
