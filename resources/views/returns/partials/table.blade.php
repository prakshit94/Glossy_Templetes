...
<x-ui.table>
    <x-ui.table-header class="bg-muted/20">
        <x-ui.table-row class="border-b border-border/60">
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Return</x-ui.table-head>
    ...
    <x-ui.table-body>
        @forelse($rows as $record)
            ...
            <x-ui.table-row class="border-b border-border/40 group hover:bg-primary/[0.02] transition-colors">
                <x-ui.table-cell>
                    <span class="text-sm font-mono font-black text-primary">{{ data_get($r, 'number') ?? data_get($r, 'return_number', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-bold">{{ data_get($r, 'order_number') ?? data_get($r, 'order.no', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-medium">{{ data_get($r, 'customer_name', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs text-muted-foreground line-clamp-2 max-w-[180px]">{{ data_get($r, 'reason', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php $rq = data_get($r, 'requested_at') ?? data_get($r, 'created_at'); @endphp
                    <span class="text-xs font-bold">{{ $rq ? \Illuminate\Support\Carbon::parse($rq)->format('M j, Y') : '—' }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php
            ...
            <x-ui.table-row>
                <x-ui.table-cell colspan="7" class="h-60 text-center">
                    <div class="flex flex-col items-center justify-center gap-3 opacity-40">
                        ...
                        <p class="text-sm font-black uppercase tracking-[0.2em]">No returns found</p>
