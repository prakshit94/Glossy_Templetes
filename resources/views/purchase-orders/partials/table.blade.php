...
<x-ui.table>
    <x-ui.table-header class="bg-muted/20">
        <x-ui.table-row class="border-b border-border/60">
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">PO #</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Supplier</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 text-right">Total</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Expected</x-ui.table-head>
    ...
    <x-ui.table-body>
        @forelse($rows as $record)
            ...
            <x-ui.table-row class="border-b border-border/40 group hover:bg-primary/[0.02] transition-colors">
                <x-ui.table-cell>
                    <span class="text-sm font-mono font-black">{{ data_get($r, 'number') ?? data_get($r, 'po_number', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-bold">{{ data_get($r, 'supplier_name') ?? data_get($r, 'supplier.name', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell class="text-right">
                    <span class="text-sm font-black tabular-nums">${{ number_format((float) data_get($r, 'total', 0), 2) }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php $ex = data_get($r, 'expected_at') ?? data_get($r, 'expected_date'); @endphp
                    <span class="text-xs font-bold">{{ $ex ? \Illuminate\Support\Carbon::parse($ex)->format('M j, Y') : '—' }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    @php
            ...
            <x-ui.table-row>
                <x-ui.table-cell colspan="6" class="h-60 text-center">
                    <div class="flex flex-col items-center justify-center gap-3 opacity-40">
                        ...
                        <p class="text-sm font-black uppercase tracking-[0.2em]">No purchase orders found</p>
