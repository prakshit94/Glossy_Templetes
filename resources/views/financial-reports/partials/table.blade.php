    ...
    <x-ui.table-body>
        @forelse($rows as $record)
            ...
            <x-ui.table-row class="border-b border-border/40 group hover:bg-primary/[0.02] transition-colors">
                ...
                <x-ui.table-cell>
                    <span class="text-xs font-bold text-muted-foreground">{{ data_get($r, 'period') ?? data_get($r, 'range', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                ...
                <x-ui.table-cell>
                    <span class="text-[10px] font-black uppercase tracking-widest">{{ data_get($r, 'format') ?? data_get($r, 'export_format', 'PDF') }}</span>
                </x-ui.table-cell>
            ...
            <x-ui.table-row>
                <x-ui.table-cell colspan="5" class="h-60 text-center">
                    <div class="flex flex-col items-center justify-center gap-3 opacity-40">
                        ...
                        <p class="text-sm font-black uppercase tracking-[0.2em]">No financial reports yet</p>
