                ...
                <x-ui.card-header class="border-b border-border/40 bg-muted/10 p-6">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div class="flex items-center gap-4">
                            <div class="size-12 rounded-xl bg-amber-500/10 text-amber-500 flex items-center justify-center shadow-inner">
                    ...
                    <x-ui.card-content class="p-0">
                        <div class="p-8 border-b border-border/40 bg-muted/5">
                            <div class="max-w-xl space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-[0.25em] text-muted-foreground/80">Role Identifier</label>
                                <input type="text" name="name" value="{{ old('name', $role->name) }}" required 
                                    class="w-full px-5 py-3.5 rounded-xl bg-background/50 border border-border/60 focus:bg-background focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all font-bold text-base tracking-tight shadow-inner text-foreground">
                                            ...
                                            <div class="flex flex-wrap gap-3">
                                                @foreach($groupPermissions as $permission)
                                                    <label x-show="isMatch('{{ $group }}', '{{ $permission->name }}')" 
                                                        ...
                                                        x-bind:class="'{{ in_array($permission->name, $rolePermissions) }}' ? 'bg-primary/10 border-primary/30 shadow-sm shadow-primary/5' : 'bg-muted/10 border-border/40 hover:border-primary/30 hover:bg-primary/5'">
