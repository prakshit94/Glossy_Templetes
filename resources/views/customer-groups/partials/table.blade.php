...
<x-ui.table>
    <x-ui.table-header class="bg-muted/20">
        <x-ui.table-row class="border-b border-border/60">
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Group</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Description</x-ui.table-head>
            <x-ui.table-head class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 text-center">Members</x-ui.table-head>
                ...
                <x-ui.table-cell>
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex flex-col min-w-0">
                            <span class="text-sm font-black tracking-tight text-foreground truncate">{{ data_get($r, 'name') ?? data_get($r, 'title', '—') }}</span>
                            <span class="text-[9px] font-mono font-bold text-muted-foreground/40 truncate">{{ data_get($r, 'code', '—') }}</span>
                        </div>
                    </div>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <span class="text-xs font-medium text-muted-foreground line-clamp-2 max-w-xs">{{ data_get($r, 'description', '—') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell class="text-center">
                    <span class="text-sm font-black tabular-nums">{{ data_get($r, 'members_count') ?? data_get($r, 'member_count', '0') }}</span>
                </x-ui.table-cell>
                <x-ui.table-cell>
                    <div class="flex items-center gap-2">
                        <x-ui.icon name="clock" size="3.5" class="text-muted-foreground/40 shrink-0" />
                        <span class="text-xs font-bold text-foreground/90">
                            @php $u = data_get($r, 'updated_at'); @endphp
                            {{ $u ? \Illuminate\Support\Carbon::parse($u)->format('M j, Y') : '—' }}
                        </span>
                    </div>
                </x-ui.table-cell>
                <x-ui.table-cell>
            ...
            <x-ui.table-row>
                <x-ui.table-cell colspan="6" class="h-60 text-center">
                    <div class="flex flex-col items-center justify-center gap-3 opacity-40">
                        ...
                        <p class="text-sm font-black uppercase tracking-[0.2em]">No customer groups found</p>
